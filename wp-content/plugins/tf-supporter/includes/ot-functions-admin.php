					...
					array(
						...
						'pages' => array(
							array(
								...
								'page_title'      => apply_filters( 'ot_theme_options_page_title', esc_html__( 'ThemeFood Options', 'option-tree' ) ),
								'menu_title'      => apply_filters( 'ot_theme_options_menu_title', esc_html__( 'ThemeFood', 'option-tree' ) ),
								'capability'      => $caps,
								'menu_slug'       => apply_filters( 'ot_theme_options_menu_slug', 'themefood-options' ),
								'icon_url'        => apply_filters( 'ot_theme_options_icon_url', get_template_directory_uri() . '/img/icon-themefood.svg' ),
								'position'        => apply_filters( 'ot_theme_options_position', 26 ),
								'updated_message' => apply_filters( 'ot_theme_options_updated_message', esc_html__( 'ThemeFood Options berhasil diupdate. Silahkan clear cache dahulu jika ada plugin cache yang aktif untuk melihat perubahan.', 'option-tree' ) ),
								'reset_message'   => apply_filters( 'ot_theme_options_reset_message', esc_html__( 'ThemeFood berhasil direset.', 'option-tree' ) ),
								'button_text'     => apply_filters( 'ot_theme_options_button_text', esc_html__( 'Simpan Perubahan', 'option-tree' ) ),
...
if ( ! function_exists( 'ot_after_theme_options_save' ) ) {
	...
	function ot_after_theme_options_save() {
		...
		if ( apply_filters( 'ot_theme_options_menu_slug', 'themefood-options' ) === $page && $updated ) {
...
if ( ! function_exists( 'ot_admin_scripts' ) ) {
	...
	function ot_admin_scripts() {
		...
		$localized_array = array(
			...
			'upload_text'           => apply_filters( 'ot_upload_text', __( 'Pilih Gambar', 'option-tree' ) ),
				...
				$settings = array(
					...
					'settings' => array(
						array(
							...
							'desc'      => esc_html__( 'Format database belum support emoji. Silahkan install plugin <a href="https://wordpress.org/plugins/database-collation-fix/" target="_blank">Database Collation Fix</a> terlebih dahulu agar theme options di halaman ini dapat tampil. Setelah itu anda dapat menjalankan proses konversi format database dengan plugin tersebut pada menu Tools (Peralatan) - Collation Fix. Jika sudah silahkan refresh kembali halaman ini.', 'option-tree' ),
							'section'   => 'general',
							'type'      => 'textblock',
							'std'       => '',
							'class'     => '',
...
if ( ! function_exists( 'ot_modify_layouts' ) ) {
	...
	function ot_modify_layouts() {
		...
		if ( isset( $_POST['option_tree_modify_layouts_nonce'] ) && wp_verify_nonce( $_POST['option_tree_modify_layouts_nonce'], 'option_tree_modify_layouts_form' ) ) { // phpcs:ignore
			...
			// Redirect.
			if ( isset( $_REQUEST['page'] ) && apply_filters( 'ot_theme_options_menu_slug', 'themefood-options' ) === $_REQUEST['page'] ) {
