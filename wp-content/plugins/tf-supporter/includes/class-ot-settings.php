			...
			foreach ( (array) $this->options as $option ) {
				...
				foreach ( (array) $this->get_pages( $option ) as $page ) {
					...
					if ( ! isset( $page['hidden_page'] ) && $screen->id === $this->page_hook[ $page['id'] ] ) {
						...
						echo '<img src="'.get_template_directory_uri().'/img/logo-themefood.png" />';
						...
						<link rel="stylesheet" href="<?=get_template_directory_uri() . '/css/select2.min.css';?>" />  
						...
						jQuery(function ($) {							
							subdistrict = [];
							$.getJSON('<?=get_template_directory_uri()?>/inc/subdistrict.json', function(result) {
								$.each(result, function(ind, val) {
								  subdistrict.push(val);
								});
							}); 
							$('#kecamatan_asal').autocomplete({
							  minLength: 2,
							  source: subdistrict,
							  select: function(event, ui) {
								$('#kecamatan_asal').val(ui.item.label);
								$('#kecamatan_id').val(ui.item.id);										
							  }
							}); 	
							$("textarea#format_order, textarea#order_diterima, textarea#order_diproses, textarea#order_diantar, textarea#order_selesai, textarea#order_dibatalkan").emojioneArea({
							  pickerPosition: "bottom",
							  tones: false,
							  search: false
							});
							if($('#setting_tombol_cta .option-tree-setting-wrap > li').length == 3) {
								$('#setting_tombol_cta .option-tree-list-item-add').hide();
							}
							$(document).on('input change', 'div[id^="setting_tombol_cta_cta"] input, div[id^="setting_tombol_cta_cta"] select', function() {
								var selectVal = $(this).parents('.option-tree-setting-body').find('div[id^="setting_tombol_cta_cta"] select option:selected').text();
								$(this).parents('.option-tree-setting').find('.open').text(selectVal);
								$(this).parents('.option-tree-setting-body').find('div[id^="setting_tombol_cta_title"] input').val(selectVal);
							});
							$(document).on('input change', 'div[id^="setting_data_bank_bank"] input', function() {
								var inputVal = $(this).parents('.option-tree-setting-body').find('div[id^="setting_data_bank_bank"] input').val();
								$(this).parents('.option-tree-setting').find('.open').text(inputVal);
								$(this).parents('.option-tree-setting-body').find('div[id^="setting_data_bank_title"] input').val(inputVal);
							});
							$(document).on('input change', 'div[id^="setting_api_open_route_token_key"] input', function() {
								var inputVal = $(this).parents('.option-tree-setting-body').find('div[id^="setting_api_open_route_token_key"] input').val();
								$(this).parents('.option-tree-setting').find('.open').text(inputVal);
								$(this).parents('.option-tree-setting-body').find('div[id^="setting_api_open_route_title"] input').val(inputVal);
							});
							$(document).on('input change', 'div[id^="setting_lokasi_toko_nama_lokasi"] input', function() {
								var inputVal = $(this).parents('.option-tree-setting-body').find('div[id^="setting_lokasi_toko_nama_lokasi"] input').val();
								$(this).parents('.option-tree-setting').find('.open').text(inputVal);
								$(this).parents('.option-tree-setting-body').find('div[id^="setting_lokasi_toko_title"] input').val(inputVal);
							});
							$(document).on('input change', 'div[id^="setting_jam_timeslot_slot_mulai"] select, div[id^="setting_jam_timeslot_slot_hingga"] select', function() {
								var selectVal = $(this).parents('.option-tree-setting-body').find('div[id^="setting_jam_timeslot_slot_mulai"] select option:selected').text();
								var selectVal2 = $(this).parents('.option-tree-setting-body').find('div[id^="setting_jam_timeslot_slot_hingga"] select option:selected').text();
								$(this).parents('.option-tree-setting').find('.open').text(selectVal + ' - ' + selectVal2);
								$(this).parents('.option-tree-setting-body').find('div[id^="setting_jam_timeslot_title"] input').val(selectVal + ' - ' + selectVal2);
							});
							$('#order_diterima_waktu_1, #order_diterima_waktu_2, #order_diterima_waktu_3').keyup(function(e) {
							  if (/\D/g.test(this.value)) {
								this.value = this.value.replace(/\D/g, '');
							  }
							  if (parseInt(this.value) > parseInt(168)) {
								$(this).val('168');
							  }
							});
							$("#form_generator").select2({
								multiple: true,
								allowClear: true,
								placeholder: "Pilih Produk"
							});
							$("#form_generator").val('').trigger('change')
							$("#form_generator option[value='']").remove();
							$('#form_generator').on("select2:select", function(e) { 
								$('#form_shortcode').val('[tf_form id="'+$(this).val().filter(Boolean)+'"]');
							});
							$('#form_generator').on('select2:unselect', function () {
								$('#form_shortcode').val('[tf_form id="'+$(this).val().filter(Boolean)+'"]');
							});
							$("#form_shortcode").focus(function() {
								var $this = $(this);
								$this.select();
