  ...
  OT_UI = {
    ...
    init_add: function() {
      ...
      $(document).on('click', '.option-tree-list-item-add', function(e) {
		...
		var listCta = $(this).prev('ul[data-name=tombol_cta]');
        OT_UI.add(this,'list_item');
		if($('> li', listCta).length >= 2) $('#setting_tombol_cta .option-tree-list-item-add').hide();
      });
    ...
    init_remove: function() {
      $(document).on('click', '.option-tree-setting-remove', function(event) {
        ...
        if (agree) {
          ...
          var listCta = $(this).parents('ul[data-name=tombol_cta]');
          OT_UI.remove(this);
		  setTimeout(function() { 
			if($('#setting_tombol_cta ul[data-name=tombol_cta] > li').length < 3) {
				$('#setting_tombol_cta .option-tree-list-item-add').show();
			} else {
